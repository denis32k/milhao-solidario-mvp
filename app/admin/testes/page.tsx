...
export default async function AdminTestesPage({ searchParams }: { searchParams: AdminSearchParams }) {
  ...
  return <main className="admin-saas-main min-h-screen px-3 py-4 lg:px-5"><div className="mx-auto max-w-6xl"><AdminPageHeader secret={secret} active="testes" title="Área de testes" description="Dados de teste/dev isolados de venda real. Teste não deve entrar no ranking nem no mapa público." />
    ...
    <section className="grid gap-3 md:grid-cols-2">{tests.map((t: any) => <article key={t.id} className="rounded-3xl bg-white p-5 shadow-xl"><p className="text-xs font-black uppercase text-purple-700">TESTE / DEV • {t.kind}</p><h2 className="mt-1 text-xl font-black text-slate-950">{t.title || t.displayName || "Teste"}</h2><p className="mt-1 text-xs font-bold text-slate-500">Criado: {dateTime(t.createdAt)} • Pedido: {shortId(t.transactionId)}</p><p className="mt-2 rounded-2xl bg-slate-50 p-3 text-xs font-bold text-slate-600">Blocos: {(t.blocks || []).map((b: any) => `x${b.gridX}/y${b.gridY}`).join(", ") || "sem blocos listados"}</p></article>)}{tests.length === 0 && <p className="rounded-3xl bg-white p-5 text-sm font-bold text-slate-500 shadow">Nenhum teste criado.</p>}</section>
  </div></main>;
}
