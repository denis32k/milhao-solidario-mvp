...
async function saveOperationalSettings(formData: FormData) {
  ...
  const settings = normalizeOperationalSettings({
    ...
    purchaseTutorialEnabled: formData.get("purchaseTutorialEnabled") === "on",
  ...
  return <main className="admin-saas-main min-h-screen px-3 py-4 lg:px-5"><div className="mx-auto max-w-6xl"><AdminPageHeader secret={secret} active="configuracoes" title="Configurações" description="Controle operacional do site sem mexer em código. A estrutura do grid fica totalmente travada." />
    ...
    <form action={saveOperationalSettings} className="mb-6 rounded-3xl bg-white p-5 shadow-xl">
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        ...
        <label className="rounded-2xl border border-slate-200 p-4"><input name="publicLinksEnabled" type="checkbox" defaultChecked={settings.publicLinksEnabled} className="mr-2" /><span className="text-sm font-black text-slate-950">Permitir links públicos</span><p className="mt-1 text-xs font-bold text-slate-500">Se desligar, cliente não consegue enviar link novo.</p></label>        <label className="rounded-2xl border border-slate-200 p-4"><input name="purchaseTutorialEnabled" type="checkbox" defaultChecked={settings.purchaseTutorialEnabled} className="mr-2" /><span className="text-sm font-black text-slate-950">Tutorial inicial</span><p className="mt-1 text-xs font-bold text-slate-500">Mostra ou remove o guia da primeira visita.</p></label>
      </div>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <label className="block"><span className="text-xs font-black uppercase text-slate-500">Tempo de reserva em minutos</span><input name="reservationMinutes" type="number" min="2" max="2" defaultValue={settings.reservationMinutes} className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm font-bold" /></label>
