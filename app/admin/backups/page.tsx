...
export default async function AdminBackupsPage({ searchParams }: { searchParams: AdminSearchParams }) {
  ...
  return <main className="admin-saas-main min-h-screen px-3 py-4 lg:px-5"><div className="mx-auto max-w-6xl"><AdminPageHeader secret={secret} active="backups" title="Backups e recuperação" description="Controle operacional de backups, exportações manuais e lembretes de restauração." />
    ...
    <section className="rounded-3xl bg-white p-5 shadow-xl"><h2 className="text-xl font-black text-slate-950">Histórico de backups</h2><div className="mt-4 space-y-2">{backups.map((b: any) => <article key={b.id} className="rounded-2xl bg-slate-50 p-3"><p className="text-xs font-black uppercase text-slate-500">{b.type} • <AdminStatusBadge value={b.status} /> • {dateTime(b.createdAt)}</p><p className="mt-1 text-sm font-bold text-slate-700">{b.note || "sem nota"}</p>{b.fileUrl && <p className="mt-1 text-xs font-bold text-slate-500">Arquivo: {b.fileUrl}</p>}</article>)}{backups.length === 0 && <p className="text-sm font-bold text-slate-500">Nenhum registro ainda.</p>}</div></section>
  </div></main>;
}
